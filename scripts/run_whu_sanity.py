...
CONFIG_PATH = PROJECT_ROOT / "configs" / "whu_unet_sanity.yaml"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "sanity_whu_unet"
VIZ_DIR = OUTPUT_DIR / "visualizations"
LOG_DIR = PROJECT_ROOT / "logs" / "train_logs"
TB_DIR = LOG_DIR / "whu_unet_sanity_tensorboard"
...
def main() -> None:
    ...
    logger = setup_logger("whu_unet_sanity", LOG_DIR / "whu_unet_sanity.log")
    ...
    logger.info("Starting WHU sanity run on device=%s", device)
    ...
    with torch.no_grad():
        for idx in sample_indices:
            ...
            pred = (torch.sigmoid(logits) >= 0.5).float().cpu()[0]
            pred_fg_ratio = float(pred.mean().item())
            pred_fg_ratios.append(pred_fg_ratio)

            out_path = VIZ_DIR / f"whu_val_pred_{idx:02d}_{sample['id']}.png"
    ...
    verdict = "✅ 正常" if (loss_decreased and metrics_available and ckpt_load_ok and all_black < len(pred_fg_ratios) and all_white < len(pred_fg_ratios)) else "⚠️ 需人工复查"
    ...
    print(f"Report: {OUTPUT_DIR / 'sanity_report.md'}")
