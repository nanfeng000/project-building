...
CONFIG_PATH = PROJECT_ROOT / "configs" / "whu_v2lite.yaml"
...
def main() -> None:
    ...
    tb_dir = ensure_dir(log_dir / "whu_v2lite_tensorboard")

    logger = setup_logger("whu_v2lite", log_dir / "whu_v2lite.log")
    ...
    logger.info("Starting WHU v2-lite baseline on device=%s", device)
    ...
    test_dataset = build_dataset("whu", "test", manifest_path=cfg["dataset"]["test_manifest"], use_augment=False)
    fg_threshold = cfg["test"].get("foreground_prefetch_ratio", 0.005)
    candidate_indices = [idx for idx, row in enumerate(test_dataset.samples) if float(row.get("fg_ratio", 0.0)) > fg_threshold]
    ...
    report_lines += ["", "## Test Visualizations", ""]
    for name, ratio in zip(viz_files, pred_ratios):
        report_lines.append(f"- `test_visualizations/{name}` (pred fg {ratio * 100:.2f}%)")
    report_lines += ["", "## Output Files", "", "- `checkpoints/best.pth`", "- `checkpoints/last.pth`", "- `history.json`", "- `test_metrics.json`", "- `curves/curve_loss.png`", "- `curves/curve_val_metrics.png`", "- `test_visualizations/*.png`"]
